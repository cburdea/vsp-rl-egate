...
def create_env(n_jobs=213,_input=None, test=True):

    class Env(object):
        def __init__(self,n_jobs=213,_input=None,raw=None):
            self.n_jobs = n_jobs
            if _input == None:
                if test:
            ...
            for i,(tour,tour_state) in enumerate(zip(tours,states)):
                for j,(index,s) in enumerate(zip(tour,tour_state[1:])):
                    ...
                    nodes[loc,:] = [job['weight']/CAP,s['weight']/CAP,s['dist']/MAX_DIST,
                                    s['time']/MAX_DIST,job['tw']['start']/MAX_DIST,
...
def create_batch_env(batch_size=batch_size,n_jobs=213, test=True):
...
def create_replay_buffer(n_jobs=213):
...
def roll_out(model,envs,states,n_steps=10,_lambda=0.213,batch_size=batch_size,n_remove=10,is_last=False,greedy=False):
...
def eval_random(epochs,envs,n_steps=10):
    ...
    def eval_once(epoch,n_instance=8,n_steps=n_steps,batch_size=batch_size,alpha=1.0):
...
def random_init(envs,n_steps,n_instance=8):
    nodes,edges = envs.reset()
    for i in range(n_steps):
        actions = [ random.sample(range(0, N_JOBS),10) for i in range(n_instance) ]
        actions = np.array(actions)
...
def train(model,envs,epochs,n_rollout,rollout_steps,train_steps,n_remove=10):
