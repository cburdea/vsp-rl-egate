...
def create_env(n_jobs,_input=None):

    class Env(object):
        def __init__(self,n_jobs=99,_input=None,raw=None):
            self.n_jobs = n_jobs
            if _input == None:
...
def roll_out(model,envs,states,n_steps=10,_lambda=0.99,batch_size=batch_size,n_remove=10,is_last=False,greedy=False):
    buffer = create_replay_buffer(99)
    with torch.no_grad():
        model.eval()
        nodes,edges = states
        _sum = 0
...
def eval_random(epochs,envs,n_steps=10):
    ...
    def eval_once(epoch,n_instance,n_steps=n_steps,batch_size=batch_size,alpha=1.0):
    ...
    print ("<<<<<<<<<<===================== random mean cost:",np.mean([eval_once(i,64) for i in range(epochs)]))

def random_init(envs,n_steps,n_instance):
    nodes,edges = envs.reset()
    for i in range(n_steps):
        actions = [ random.sample(range(0, N_JOBS),10) for i in range(n_instance) ]
        actions = np.array(actions)
...
#         states,mean_cost = random_init(envs,pre_steps)
        ...
        with open(r'log.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, before_cost, cost])
