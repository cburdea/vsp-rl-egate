...
#                 print (hx.size(),encoder_inputs.size(),mask.size())
    ...
    def sample_greedy(self,encoder_inputs,hx,n_steps,num_samples):
        best_log_p = None
        best_actions = None
        for i in range(num_samples):
            actions,log_p,_ = self(encoder_inputs,hx,n_steps)
#                 print (best_log_p)
#                 print (log_p)
            if best_log_p is None:
                ...
                mask = (log_p > best_log_p)
                best_log_p = best_log_p.masked_scatter(mask,log_p.masked_select(mask))
                mask = mask.unsqueeze(-1)
                best_actions = best_actions.masked_scatter(mask,actions.masked_select(mask))

        return best_actions,best_log_p,torch.tensor(0.0)

class Model(nn.Module):
    def __init__(self,input_node_dim,hidden_node_dim,input_edge_dim,hidden_edge_dim,conv_layers=2):
        super(Model,self).__init__()
        self.encoder = Encoder(input_node_dim,hidden_node_dim,input_edge_dim,hidden_edge_dim,conv_layers=conv_layers)
    ...
    def forward(self,datas,steps,greedy=False,num_samples=8):
